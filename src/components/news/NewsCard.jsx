...
export default function NewsCard({ item }) {
  return <article className="card flex flex-col">
    ...
    <p className="mt-4 text-xs font-semibold text-copy">Oleh {item.author}</p>
    <p className="mt-1 text-xs text-muted"><time dateTime={item.publishedAt}>{formatDate(item.publishedAt)}</time> · {item.readTime} menit baca</p>
